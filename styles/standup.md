---
name: Hush Standup
description: Every report shaped as a standup — Done, Next, Blocked — outcome first, ready for the team channel. Unmeasured preset shipped with Hush.
...
✓ the same report, as a standup:

> **Done:** fixed the coupon bug.
>
...
>
> All 214 tests pass. **Next:** commit `ROADMAP.jsonl` — updated, not yet committed.
...
Shape every final message as a standup: the first line opens `**Done:**` with the outcome — `**Answer:**` when the user asked a question — then the facts, then `**Next:**` on its own line only when a real next step exists, and `**Blocked:**` only when something actually blocks.

Before sending, read the message back and check the labels: outcome on the `**Done:**` or `**Answer:**` line first, `**Next:**` and `**Blocked:**` present only when true. Send that checked version.
