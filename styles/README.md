...
### Anchor
_ADHD Tailored, Chunked, signposted, low memory load._

>**Root cause: `netPay` withheld tax before subtracting the pre-tax deduction, taxing income the deduction should have shielded.**
>
>`src/payroll.js:11-12` called `withholdTax` on the full gross, then subtracted the deduction afterward — so the deduction never lowered taxable income, it just chopped a fixed dollar >amount off an already-taxed number. That undershot every deduction case by `deduction × TAX_RATE` (e.g. $20 short on a $100 deduction).
>
>Fixed by reordering: subtract the deduction from gross first, then withhold tax on the resulting taxable amount.
>
>Tests: 3/3 pass, 92ms.
